...
if __name__ == "__main__":
    ...
    llm_model = AutoModelForCausalLM.from_pretrained(config.ct2_dir).to(device)

    from tsllm.llm.text_generation import llm_gen_hf

    llm_gen_fn = llm_gen_hf
    from tsllm.llm.llm_call_with_cache import llm_forward_fn_with_cache, token_value_fn_with_cache
    def llm_forward_fn(prompt: str, past_key_values=None):
        return llm_forward_fn_with_cache(
            model=llm_model,
            tokenizer=tokenizer,
            prompt=prompt,
    ...
    # policy_forward_value = partial(token_value_fn_with_cache, critic, tokenizer)
    ...
    def mcts_multi_search(
        ...
        ts_llm_gen_fn = llm_gen_fn
        ts_policy_forward_value = policy_forward_value
