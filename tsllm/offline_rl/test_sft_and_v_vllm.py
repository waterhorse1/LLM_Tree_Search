...
if __name__ == "__main__":
    ...
    policy_model_name = "llama2-7b-game24-sft-ep3-hf"
    value_model_name = "llama2-7b-game24-value-sft-ep3"
    controller_addr = "http://localhost:21101"

    llm_gen_fn = partial(
        llm_gen_with_logp_vllm, policy_model_name, None, None, unused_stop=None
    )
    policy_forward_value = partial(_value_inference_fastchat, value_model_name, None)
    ...
    def cot_direct_output(args, problem_inst, stop, **kwargs):
        ...
        texts, logps = llm_gen_fn(
            prompt=prompt, num_sequence=1, max_new_tokens=max_new_tokens, **kwargs
        )
    ...
    def mcts_multi_search(
        ...
        value_worker_addr = get_worker_address(value_model_name, controller_addr)
        policy_worker_addr = get_worker_address(policy_model_name, controller_addr)
        # value_worker_addr =  None
        # policy_worker_addr = None

        ts_llm_gen_fn = partial(
            llm_gen_fn,
            worker_addr=policy_worker_addr,
        )

        ts_policy_forward_value = partial(
            policy_forward_value, worker_addr=value_worker_addr
        )
        env = task_module.Env(
            config={
        ...
        if args.rollout_method == "mcts.rollout":
            ...
            if len(texts) > 0:
                value_list = ts_policy_forward_value(
            ...
            output_list = mcts.beam_search(
                env, args.num_mcts_aggregation, args.max_length, ts_policy_forward_value
            )
    ...
    def test_problem(
        args,
        idx_and_problem_inst,
        cot_writer,
        cot_sc_writer,
        mcts_no_term_writer,
        mcts_w_term_writer,
    ):
        idx, problem_inst = idx_and_problem_inst
        results = {}
    ...
    for i_arg, cur_args in enumerate(args_list):
        ...
        if TEST_COT_GREEDY:
            ...
            if not cot_save_path.exists():
                cot_save_path.mkdir(parents=True)
            cot_writer = jsonlines.open(cot_save_path / "merged.jsonl", "a")
        else:
            cot_writer = None
        if TEST_COT_SC:
            cot_sc_save_path = writer_dir / "cot_sc"
            if not cot_sc_save_path.exists():
                cot_sc_save_path.mkdir(parents=True)
            cot_sc_writer = jsonlines.open(cot_sc_save_path / "merged.jsonl", "a")
        else:
            cot_sc_writer = None

        if TEST_NO_TERMINAL:
            mcts_no_term_save_path = writer_dir / "no_terminal_reward"
            if not mcts_no_term_save_path.exists():
                mcts_no_term_save_path.mkdir(parents=True)
            mcts_no_term_writer = jsonlines.open(
                mcts_no_term_save_path / "merged.jsonl", "a"
            )
        else:
        ...
        t0 = time.time()
        with ThreadPoolExecutor(config.num_workers) as pool:
            test_fn = partial(
                test_problem,
                args,
                cot_writer=cot_writer,
                cot_sc_writer=cot_sc_writer,
                mcts_no_term_writer=mcts_no_term_writer,
                mcts_w_term_writer=mcts_w_term_writer,
            )
            test_inputs = [(i, test_ds[i]) for i in range(len(test_ds))]
            output_list = pool.map(test_fn, test_inputs)
            for results in (pbar := tqdm(output_list, total=len(test_ds))):
                for k, v in results.items():
                    if isinstance(v, int):
